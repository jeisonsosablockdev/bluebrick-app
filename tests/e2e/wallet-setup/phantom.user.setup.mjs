...
export default defineWalletSetup(walletPassword, async (context, walletPage) => {
  ...
  const walletProfile = tryLoadWalletProfile("user");
  ...
  await phantom.importWalletFromPrivateKey("solana", walletProfile.privateKeyBase58, "user-wallet");
  await phantom.switchAccount("user-wallet");
});
