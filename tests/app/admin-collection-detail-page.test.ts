...
describe("app/admin/collections/[id]/page", () => {
  ...
  it("renders the success handoff for a ready entry", async () => {
    ...
    expect(html).toContain("Detail editor");
    expect(html).toContain("Content workspace");
    expect(html).toContain("Blockchain reference");
    expect(html).not.toContain("Summary editor mounted");
    expect(html).not.toContain("Property information editor mounted");
    expect(html).not.toContain("Gallery tabs shell mounted");
    expect(html).not.toContain("Documents editor mounted");
    expect(html).not.toContain("Blockchain addresses panel mounted");
    expect(html).not.toContain("Mounting plan");
    ...
    expect(html.indexOf("Fractional investment summary")).toBeLessThan(html.indexOf("Blockchain panel"));
    expect(html).toContain("Back to collections");
  });
});
