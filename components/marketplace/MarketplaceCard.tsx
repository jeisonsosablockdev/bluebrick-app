...
export function MarketplaceCard({ property, onOpenDetail }: MarketplaceCardProps) {
  return (
    <Card className="overflow-hidden p-0">
      <button type="button" className="block w-full text-left" onClick={() => onOpenDetail(property.id)} aria-label={`Ver detalle de ${property.title}`}>
      ...
      <div className="space-y-2 p-4">
        ...
        <p className="text-sm text-cyan-200">Precio NFT: ${property.nftPriceUsd.toFixed(2)}</p>
        <p className="text-sm font-semibold text-cyan-300">ROI estimado: {property.annualRoiPct.toFixed(1)}%</p>
