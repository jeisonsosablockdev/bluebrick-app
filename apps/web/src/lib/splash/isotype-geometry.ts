/**
 * @file apps/web/src/lib/splash/isotype-geometry.ts
 * @description Layer 3: Domain / Pipelines - Canonical decomposed SVG geometry and metadata
 * for the 4 animatable segments of the BlueBrick brand mark.
 */

import type { IsotypePieceConfig, IsotypePieceId } from "./types";

/**
 * Standard SVG viewBox coordinates matching the canonical brand mark vector assets.
 */
export const ISOTYPE_VIEWBOX = "0 0 160 168";

/**
 * Default canvas dimensions (width and height in px) for the isotype vector.
 */
export const ISOTYPE_DEFAULT_SIZE = 160;

/**
 * Primary and secondary color tokens for the isotype pieces.
 */
export const ISOTYPE_COLORS = {
  whitePrimary: "#FFFFFF",
  whiteSecondary: "#04283C", // High-contrast metallic deep navy for 3D flip
  redPrimary: "#FC040C",     // Canonical BlueBrick crimson red accent
  redSecondary: "#E0030A",   // Deeper red accent for 3D flip highlight
} as const;

/**
 * Step 1: Decomposed path definitions and animation sequencing for each of the 4 brand segments.
 * Pieces are ordered strictly from left to right (chronological entrance sequence).
 */
export const ISOTYPE_PIECES: readonly IsotypePieceConfig[] = [
  {
    id: "small_white",
    name: "Lower-left small white anchor bar",
    pathData:
      "M 14 88.661 C 4.703 92.037, 1 95.795, 1 101.856 C 1 105.355, 25.743 156.702, 28.593 159.116 C 35.277 164.778, 51 157.678, 51 148.998 C 51 144.921, 25.935 93.027, 22.906 90.833 C 20.146 88.834, 16.185 87.868, 14 88.661",
    initialFill: ISOTYPE_COLORS.whitePrimary,
    secondaryFill: ISOTYPE_COLORS.whiteSecondary,
    order: 0,
    delaySeconds: 0.0,
  ...
  {
    id: "large_white_left",
    name: "Central-left large diagonal white bar",
    pathData:
      "M 28 24.344 C 17.794 28.680, 14.048 31.007, 12.521 33.960 C 9.277 40.234, 7.294 35.591, 58.246 141.057 C 71.123 167.712, 72.398 168.527, 88.765 160.561 C 98.348 155.897, 101 153.052, 101 147.435 C 101 143.659, 44.189 26.170, 41.517 24.419 C 37.687 21.910, 33.780 21.888, 28 24.344",
    initialFill: ISOTYPE_COLORS.whitePrimary,
    secondaryFill: ISOTYPE_COLORS.whiteSecondary,
    order: 1,
    delaySeconds: 0.2,
  ...
  {
    id: "accent_red",
    name: "Top-right signature red brick accent",
    pathData:
    ...
    initialFill: ISOTYPE_COLORS.redPrimary,
    secondaryFill: ISOTYPE_COLORS.redSecondary,
    order: 3,
    delaySeconds: 0.6,
    durationSeconds: 0.6,
  },
