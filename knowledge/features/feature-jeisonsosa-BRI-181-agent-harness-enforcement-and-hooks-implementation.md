...
description: Guía de implementación descompuesta en 6 SPECs atómicas para la actualización del harness, subagentes, task-init.sh, arquitectura estándar de 4 capas, desacoplamiento y protocolo de Doble Compuerta.
tags: [governance, agents, harness, implementation, clean-code, double-gatekeeper, bri-181]
...
El trabajo se descompone en 6 SPECs atómicas secuenciales creadas a partir de la rama parent `refactor/jeisonsosa-BRI-181-agent-harness-enforcement-and-hooks`:
...
### **SPEC-6**: Protocolo de Doble Compuerta para Architect (`SPEC/jeisonsosa-BRI-181-spec-6-double-gatekeeper-architect-workflow`) (En Curso)
- Formalizar en `.agents/hooks.json` y `AGENTS.md` el ciclo de Doble Compuerta:
  - **Compuerta 1 (Pre-Code Review)**: `architect` aprueba la arquitectura proyectada antes de escribir código.
  - **Compuerta 2 (Post-Code Review)**: `architect` audita el diff entregado e impide commits no conformes.
- Sincronizar la política de gobernanza en `AGENTS.md` y `knowledge/governance/git-monorepo-policy.md`.
...
- **Delivery SPEC 6**: `SPEC/jeisonsosa-BRI-181-spec-6-double-gatekeeper-architect-workflow` (Active)

## What tests go first
- Verificación sintáctica YAML de `.agents/agents/*.yaml`.
- Validación de flujo de Doble Compuerta en `.agents/hooks.json`.
