...
description: Guía de implementación descompuesta en 8 SPECs atómicas para la actualización del harness, subagentes, task-init.sh, 4 capas, desacoplamiento, doble compuerta, motor idempotente y automatización de PRs post Human Acceptance.
tags: [governance, agents, harness, implementation, clean-code, double-gatekeeper, task-lifecycle, pr-automation, bri-181]
...
El trabajo se descompone en 8 SPECs atómicas secuenciales creadas a partir de la rama parent `refactor/jeisonsosa-BRI-181-agent-harness-enforcement-and-hooks`:
...
### **SPEC-8**: Motor de Automatización de PRs post Human Acceptance (`SPEC/jeisonsosa-BRI-181-spec-8-pr-auto-engine-after-human-acceptance`) (En Curso)
- Crear el script `scripts/ci/generate-pr-body.sh` para generar el `pr-body.md` determinista tras la aprobación humana.
- Crear el script `scripts/ci/pr-auto.sh` para deducir etiquetas (`scope:*`, `type:*`, `risk:*`), ejecutar `pnpm pr:open` e inyectar etiquetas via GitHub API.
- Añadir el comando `"pr:auto": "bash ./scripts/ci/pr-auto.sh"` a `package.json`.
- Registrar la automatización en `AGENTS.md`.
...
- **Delivery SPEC 8**: `SPEC/jeisonsosa-BRI-181-spec-8-pr-auto-engine-after-human-acceptance` (Active)

## What tests go first
- Verificación del generador `scripts/ci/generate-pr-body.sh`.
- Verificación del script de automatización `scripts/ci/pr-auto.sh`.
- Gates automatizados con `pnpm validate`.

## What tooling is required
- Runner: Google Antigravity SDK con modelos Gemini.
- Scripts de CI: `scripts/ci/generate-pr-body.sh`, `scripts/ci/pr-auto.sh`, `scripts/ci/check-task-lifecycle.sh`.
