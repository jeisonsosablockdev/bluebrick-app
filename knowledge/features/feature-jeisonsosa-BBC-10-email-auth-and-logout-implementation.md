...
1. **Layer 1: Presentation**
   ...
   - `apps/web/src/components/auth/logout-confirm-modal.tsx`: Dedicated accessible modal dialog for session termination confirmation with luxury dark theme styling, animated transitions, and a "No volver a preguntar" (`dontAskAgain`) toggle.
   - `apps/web/src/components/dashboard/investment-dashboard.tsx`: Integrate explicit logout button in the header with `LogOut` icon, localized title `common.logout`, responsive user profile container (`.dash-user-text-container`), and `LogoutConfirmModal` integration with `localStorage` preference (`bluebrick_skip_logout_confirm`).
...
3. **Layer 3: Domain & Contracts**
   - `apps/web/src/features/i18n/domain/models/locale-types.ts`: Update dictionary interface with `loginCard.emailLoginButton`, `common.logout`, and `logoutModal` token contracts.
   - `apps/web/src/features/i18n/domain/schemas/i18n-dictionary-schema.ts`: Validate `LogoutModalTokensSchema` and dictionary definitions across schemas with Zod.
   - `apps/web/src/features/i18n/domain/dictionaries/{es,en,pt}.ts`: Spanish, English, and Portuguese dictionary translations for `emailLoginButton`, `logout`, and `logoutModal` (`title`, `description`, `dontAskAgain`, `confirmButton`, `cancelButton`).
