...
resource: https://github.com/jeisonsosablockdev/brids/blob/develop/knowledge/fixes/bri-165/fix-adminassetsnew-bri-165.md
---
...
- Before Slice 20, the upload contract returned `/api/admin/assets/uploads/[uploadId]/binary`, and that route read `request.arrayBuffer()` inside the Function before calling Vercel Blob; this made otherwise valid document uploads vulnerable to Vercel's Function payload limit.
- Before Slice 20, the UI surfaced `Storage upload failed (413)` after the storage step, which did not explain the `10 MB` product limit or point operators to a compression path.
...
- The upload client already accepts `editSessionId`, but the original `/admin/assets/new` workflow did not pass one until the upload lifecycle slices.
